...
const AppointmentForm = () => {
    ...
    const fetchDoctors = async () => {
      try {
        const { data } = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/api/v1/user/doctors`,
          { withCredentials: true }
        );
        setDoctors(data.doctors || []);
      } catch (error) {
        toast.error(
          error?.response?.data?.message || "Unable to load doctors. Please try later."
        );
      }
    };
    fetchDoctors();
  ...
  const handleAppointment = async (e) => {
    ...
    try {
      ...
      if (!selectedDoctor) {
        toast.error("Selected doctor is not valid. Please choose again.");
        return;
      }
          ...
          <div>
            ...
            {doctors.filter((doctor) => doctor.doctorDepartment === department)
              ...
              <p style={{ color: "red", fontSize: "0.9rem" }}>
                No doctors available for {department}. Contact admin to add.
              </p>
            )}
          </div>
          <textarea
